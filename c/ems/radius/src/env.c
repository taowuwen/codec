...
ENV *rc_new_env(int size)
{
	...
	if ((p = radius_malloc(sizeof(*p))) == NULL)
		return NULL;

	if ((p->env = radius_malloc(size * sizeof(char *))) == NULL)
	{
...
int rc_add_env(ENV *env, char *name, char *value)
{
	...
	if (env->env[i])
	{
		if ((new_env = radius_realloc(env->env[i], strlen(name)+strlen(value)+2)) == NULL)
			return -1;
		...
		if ((env->env[env->size] = radius_malloc(strlen(name)+strlen(value)+2)) == NULL) {
