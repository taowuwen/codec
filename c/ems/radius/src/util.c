...
rc_destroy(rc_handle *rh)
{
	...
	if (rh->this_host_bind_ipaddr != NULL)
		radius_free(rh->this_host_bind_ipaddr);
	if (rh->ppbuf != NULL)
		radius_free(rh->ppbuf);
	radius_free(rh);
}
...
rc_fgetln(FILE *fp, size_t *len)
{
	...
	if (buf == NULL) {
		bufsiz = RC_BUFSIZ;
		if ((buf = radius_malloc(bufsiz)) == NULL)
			return NULL;
	}
	...
	*len = 0;
	...
	while ((ptr = strchr(&buf[*len], '\n')) == NULL) {
		...
		char *nbuf = radius_realloc(buf, nbufsiz);
