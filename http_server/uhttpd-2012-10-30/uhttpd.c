...
#ifdef DEBUG
static int _log_level = LOG_TRACE;

#ifdef DT_USE_SELECT
static pthread_mutex_t mtx = PTHREAD_MUTEX_INITIALIZER;
#endif

void _uh_log(unsigned int level, 
	     const char *func, unsigned int ln, const char *fmt, ...)
{
	int rtn;
	...
	rtn = snprintf(log, 2048, "[%u][%d--%s]:\t", 
			(unsigned int )pthread_self(), ln, func);
#else

	rtn = snprintf(log, 2048, "[%s(%d)]:\t", func, ln);
#endif

	va_start(arg, fmt);
	vsnprintf(log + rtn, 2048 - rtn, fmt, arg);
	va_end(arg);

	{
		FILE *fp;
		fp = fopen("/tmp/uhttpd.log", "a+");
		if (fp) {
...
static int uh_realpath_match(const char *root, const char *prefix, const char *phys)
{
	char tmp[1024];
	char *p = NULL;
	int   ret = 0;
	...
	do {
		...
		if (!(root && prefix && phys)) break;

		snprintf(tmp, sizeof(tmp), "%s/%s", root, prefix); 

		p = realpath(tmp, NULL);
		if (!p) break;

		D("uh_realpath_match: path: %s, phys: %s", p, phys);

		if (strstr(phys, p) == phys) ret = 1;
	} while (0);
...
static bool 
uh_dispatch_request(struct client *cl, struct http_request *req, int *detached)
{
	struct path_info *pin;
#ifdef HAVE_CGI
	struct interpreter *ipr = NULL;
	static char path_real[1024] = {0};
#endif
	...
	if ((pin = uh_path_lookup(cl, req->url)) != NULL)
	{
		...
		{
#ifdef HAVE_CGI
			if (uh_realpath_match(pin->root, conf->cgi_prefix, realpath(pin->phys, path_real)) ||
...
#endif
			return uh_file_request(cl, pin, detached);
		}
	}
...
#endif
				return uh_file_request(cl, pin, NULL);
			}
		}
...
static void uh_client_cb(struct client *cl, unsigned int events)
{
	...
	if (!cl->dispatched)
	{
		...
		/* dispatch request */
		if (!uh_dispatch_request(cl, req, &detached))
		{
			if (detached) {
			} else {
				D("Client(%d) finished\n", cl->fd.fd);
