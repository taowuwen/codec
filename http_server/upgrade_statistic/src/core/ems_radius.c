...
static ems_void dev_evt_cb(ems_session *sess, ems_int err, ems_int flg)
{
	...
	if (err) {
		ems_l_trace("[dev] evt err, sess: %d %s",
...
static ems_int ra_fill_auth_avpairs(ems_device *dev, ems_radius *ra)
{
	...
	ems_mac_update(mac, cfg_get(emscfg(), CFG_ems_mac));
	snprintf(buf, sizeof(buf), "%s:%s", mac, 
			cfg_get(emscfg(), CFG_wireless_ssid));
...
#if 0
	ems_rc_avpair_add(ra->rh, &dev->vp_out, PW_CALLING_STATION_ID, 
			str_text(&dev->user.mac), str_len(&dev->user.mac), 0);
#endif

	snprintf(buf, sizeof(buf), "%s", cfg_get(emscfg(), CFG_ems_sn));
...
static ems_int ra_connect(ems_session *sess)
{
	...
	ems_l_trace("[dev] sess(%d) connect to: %s(%s): %d...",
			fd, 
			ems_sock_addr(sock), 
	...
	if (connect(fd, (struct sockaddr *)&addr, len)) {
		ems_l_trace("[dev] connect to: %s:%d: failed: %s",
				ems_sock_addr(sock), 
				ems_sock_port(sock), 
				ems_lasterrmsg());
		close(fd);
...
static ems_int ra_connect_to_acct_server(ems_device *dev, ems_session *sess, ems_radius *ra)
{
	...
	ems_send_message(ty_radius, ty_portal, EMS_APP_CMD_RADIUS_AUTH_RSP, rsp);
	json_object_put(rsp);
...
static ems_int ra_auth_rsp(ems_device *dev, ems_session *sess)
{
	...
	ems_l_trace("[radius] auth->code: %d", auth->code);

	if (auth->code == PW_ACCESS_ACCEPT) {
		ems_l_trace("user: %s@%s login success", 
		...
		ems_l_trace("*******server need user: %s@%s access challenge", 
				str_text(&dev->user.name),
				str_text(&dev->user.ip));
...
static ems_int ra_acct_rsp(ems_device *dev, ems_session *sess)
{
	...
	ems_l_trace("[radius] acct->code: %d", auth->code);

	if (auth->code == PW_ACCOUNTING_RESPONSE) {
		ems_l_trace("user: %s@%s accounting success", 
				str_text(&dev->user.name),
				str_text(&dev->user.ip));
		...
		ems_l_trace("[radius] acct failed");
...
static ems_int ra_ping_recv_handle(ems_device *dev, ems_session *sess)
{
	...
	if(icp->icmp_type != ICMP_ECHOREPLY) {
		ems_l_trace("[dev] %d bytes from: %s icmp not reply(%d)",
	...
	if( icp->icmp_id != dev->ident) {
		ems_l_trace("[dev] %d bytes icmp ident: %d error, not : %d", cc, icp->icmp_id, dev->ident);
		ems_buffer_clear(&sess->buf_in);
		return EMS_CONTINUE;
	}
...
again:
	...
	if (ret <= 0) {
		...
		default:
			ems_l_trace("[dev] ping error: %s", ems_lasterrmsg());
			return EMS_ERR;
		}
	}
...
static ems_int ra_acct_stopped_rsp(ems_device *dev, ems_session *sess)
{
	...
	if (auth->code == PW_ACCOUNTING_RESPONSE) {
		ems_l_trace("user: %s@%s accounting stopped success", 
				str_text(&dev->user.name),
				str_text(&dev->user.ip));
	}
	else {
		ems_l_trace("[radius] accounting stopped failed");
	}
...
static ems_int ra_err(ems_device *dev, ems_session *sess, ems_uint flg)
{
	...
	ems_l_trace("shutdown device: %s, %s, %s, %s", 
			str_text(&dev->user.name),
...
static ems_int ra_to_ping(ems_device *dev, ems_session *sess, ems_timeout *to)
{
	ems_l_trace("[radius] timeout dectect ping retry times: %d, disconnect: %d", dev->retry_times, dev->disconnect);

	sess_event_cancel(sess);

	if (dev->retry_times > 0) {
		dev->retry_times--;
		ems_l_trace("retry send ping: %d", dev->retry_times);
	...
	dev->disconnect--;
	if (dev->disconnect <= 0) {
		ems_l_trace("[radius] lost user: %s@%s", str_text(&dev->user.name), str_text(&dev->user.ip));
...
static ems_int ra_update_user_traffic(ems_device *dev)
{
	...
	snprintf(cmd, sizeof(cmd), "iptaccount -s -f -l locus_%s | grep %s | cut -d';' -f2-", 
...
static ems_int ra_fill_acct_avpairs(ems_device *dev, ems_radius *ra)
{
	...
	if (str_len(&dev->user.sessid) <= 0) {
		...
		snprintf(buf, sizeof(buf), "locus%u%ld%ld", 
...
static ems_int st_into_acct_stop(ems_device *dev)
{
	...
	/* send msg to portal, to disconnect with */
	{
		...
		ems_send_message(ty_radius, ty_portal, EMS_APP_CMD_PORTAL_LOGOUT, jobj);
...
static ems_int ra_ping_connect(ems_session *sess)
{
	...
	ems_l_trace("[ra] sess: %d host addr: %s", 
