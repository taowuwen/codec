...
dns_get_all_address(ems_cchar *payload, ems_cchar *rsp, struct in_addr *addr, ems_int *n_addr)
{
	...
	*n_addr = 0;
	...
	while (total > 0) {
		...
		payload = dns_get_name(payload, rsp, url);
		ems_l_trace("dns name: %s total: %d", url, total);
		...
		if (ntohs(itm->ty) == 1 && ntohs(itm->cls) == 1) {
			...
			ems_l_trace("A : %s", inet_ntoa(addr[*n_addr]));
		...
		ems_cchar *payload, dns_header *dns, ems_cchar *rsp)
{
	...
	ems_l_trace("current addr: %s, total n: %d", str_text(&url->url), n_addr);
...
fw_dns_parse_and_filter(ems_fw *fw, ems_session *sess, dns_user *user, dns_header *dns)
{
	...
	ems_l_trace("question: %s, class: 0x%x, type: 0x%x", 
	...
	if (url) {
		...
		if (ems_flag_like(emscorer()->flg, FLG_SUBDOMAIN_ENABLE) &&
			fw_dns_is_subdomain(fw, str_text(&qust.qname))) {
			url = dns_url_new();
			if (url) {
...
static ems_int fw_dns_fwd_filter(ems_fw *fw, ems_session *sess)
{
	...
	ems_l_trace("DNS response, length: %d", buf_len(&sess->buf_in));
	...
	if (user) {
		ems_l_trace("user (%s:%d) dns replied", inet_ntoa(user->addr), user->port);
...
again:
	...
	if (ret <= 0) {
		...
		default:
			ems_l_trace("[fw] fw error: %s", ems_lasterrmsg());
			return EMS_ERR;
		}
	}
...
static ems_void fw_dns_fwd_evt_cb(ems_session *sess, ems_int err, ems_int flg)
{
	...
	if (err) {
		ems_l_trace("[fw] client dns err sess: %d", ems_sock_fd(&sess->sock));
		/* do reconnect */
