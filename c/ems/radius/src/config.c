...
{
	if (p) {
		option->val = (void *) radius_strdup(p);
		if (option->val == NULL) {
...
static int set_option_int(const char *filename, int line, OPTION *option, const char *p)
{
	...
	if ((iptr = radius_malloc(sizeof(*iptr))) == NULL) {
		rc_log(LOG_CRIT, "read_config: out of memory");
		return -1;
	}
...
static int set_option_srv(const char *filename, int line, OPTION *option, const char *p)
{
	...
	if (serv == NULL) {
		rc_log(LOG_ERR, "option->val / server is NULL, allocating memory");
		serv = radius_malloc(sizeof(*serv));
		if (serv == NULL) {
			rc_log(LOG_CRIT, "read_config: out of memory");
			radius_free(p_dupe);
			return -1;
	...
	if ((q = strchr(p_pointer,':')) != NULL) {
		*q = '\0';
		...
		if((s = strchr(q,':')) != NULL) {
			*s = '\0';
			s++;
			serv->secret[serv->max] = radius_strdup(s);			
	...
	serv->name[serv->max] = radius_strdup(p_pointer);
...
rc_config_init(rc_handle *rh)
{
	...
	SERVER *acctservers;
        ...
        rh->config_options = radius_malloc(sizeof(config_options_default));
	...
	authservers = radius_malloc(sizeof(SERVER));
	acctservers = radius_malloc(sizeof(SERVER));
	...
	for (i = 0; i < NUM_OPTIONS; i++) {
		...
		if (rh->config_options[i].type == OT_SRV) {
			...
			for (j = 0; j < serv->max; j++)
				radius_free(serv->name[j]);
			radius_free(serv);
		} else {
			radius_free(rh->config_options[i].val);
		}
	}
	radius_free(rh->config_options);
