...
void rc_random_vector (unsigned char *);
int rc_check_reply (AUTH_HDR *, int, char *, unsigned char *, unsigned char);
...
int rc_pack_list (VALUE_PAIR *vp, const char *secret, AUTH_HDR *auth)
{
...
int rc_send_server (rc_handle *rh, SEND_DATA *data, char *msg)
{
	...
	rc_log(LOG_ERR, "DEBUG: rc_send_server: creating socket to: %s", server_name);
	...
	rc_log(LOG_ERR, "DEBUG: local %s : 0, remote %s : %u\n", 
...
int rc_check_reply (AUTH_HDR *auth, int bufferlen, char *secret, unsigned char *vector, uint8_t seq_nbr)
{
	int             secretlen;
	int             totallen;
