...
static ems_void dns_fwd_timeout(ems_timeout *timeout)
{
	...
	ems_l_trace("---->>>>>>>> [DNS]TIMEOUT for user(%s:%d) <<<<<<<<----", 
...
static ems_int dns_bind(ems_session *sess)
{
	...
	addr.sin_port   = htons(EMS_DNS_BIND);
...
fw_dns_fwd(ems_fw *fw, ems_session *sess, struct sockaddr_in *from)
{
	...
	ems_l_trace("DNS request from(%s:%d), length: %d",
		inet_ntoa(from->sin_addr), ntohs(from->sin_port), buf_len(&sess->buf_in));

	dns = (dns_header *)buf_rd(&sess->buf_in);

	user = dns_find_user(fw, ntohs(dns->id));
	if (user) {
		ems_l_trace("resend dns request to user: (%s: %d): 0x%x", 
		...
		ems_l_trace("new request: (%s:%d) id: 0x%x",  
				inet_ntoa(user->addr), user->port, ntohs(dns->id));
...
static ems_void fw_dns_evt_cb(ems_session *sess, ems_int err, ems_int flg)
{
	...
	if (err) {
		ems_l_trace("[fw] server dns err sess: %d", ems_sock_fd(&sess->sock));
...
static ems_int fw_dns_url_info_upt(dns_url *url)
{
	...
	if (mask) {
		...
		if (*mask) {
			...
			ems_l_trace("update address: %s --> %s, mask(%s)", addr, buf, mask);

			url->mask = ems_atoi(mask);
			if (url->mask < 0 || url->mask > 32) {
				ems_l_trace("invalid netmask: %d", url->mask);
				return EMS_ERR;
			}
