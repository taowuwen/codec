...
bool uh_socket_wait(int fd, int sec, bool write)
{
	...
	if (rv == 0) {
		D("IO: FD(%d) select timeout (rv=%d), msg: %s\n", fd, rv, strerror(errno));
	}

	if (rv < 0)
	{
		D("IO: FD(%d) appears dead (rv=%d): err: %s\n", fd, rv, strerror(errno));
		return false;
	}
						 ...
						 int (*wfn) (struct client *, const char *, int))
{
	...
	while (true)
	{
		...
		{
			DV("IO: FD(%d) short write %d/%d bytes\n", fd, rv, len);
			len -= rv;
			buf += rv;
			continue;
		}
...
int uh_tcp_recv_lowlevel(struct client *cl, char *buf, int len)
{
	...
	//return recv(cl->fd.fd, buf, len, 0);
}
...
struct path_info * uh_path_lookup(struct client *cl, const char *url)
{
	...
	{
		D("phys path: %s ok, mode: 0x%x, size: 0x%x\n",
				path_phys, p.stat.st_mode, p.stat.st_size);
		/* is a regular file */
...
void uh_client_remove(struct client *cl)
{
	...
	for (cur = uh_clients; cur; prv = cur, cur = cur->next)
	{
		if (cur == cl)
		{
			...
			uh_ufd_remove(&cur->rpipe, 1);
			uh_ufd_remove(&cur->wpipe, 1);
			uh_ufd_remove(&cur->fd, 1);
...
void uh_client_destroy(struct client *cl)
{
	if (!cl)
		return;
...
struct client *uh_client_detach(struct client *cl)
{
	struct client *cur = NULL;
	struct client *prv = NULL;
	...
	for (cur = uh_clients; cur; prv = cur, cur = cur->next)
	{
		if (cur == cl)
		{
			...
			uh_ufd_remove(&cur->rpipe, 1);
			uh_ufd_remove(&cur->wpipe, 1);
			uh_ufd_remove(&cur->fd, 0);

			cur->server->n_clients--;

			D("IO: Client(%d) detach\n", cur->fd.fd);
...
void uh_ufd_remove(struct uloop_fd *u, int doclose)
{
	if (u->cb != NULL)
	{
		uloop_fd_delete(u);
