...
struct hostent *rc_gethostbyname(const char *hostname)
{
	...
	hostbuflen = 1024;
	tmphostbuf = radius_malloc(hostbuflen);
#endif
#endif
	...
	while ((res = gethostbyname_r(hostname, &hostbuf, tmphostbuf, hostbuflen, &hp, &herr)) == ERANGE)
	{
		/* Enlarge the buffer */
		hostbuflen *= 2;
		tmphostbuf = radius_realloc(tmphostbuf, hostbuflen);
	}
	radius_free(tmphostbuf);
...
uint32_t rc_own_bind_ipaddress(rc_handle *rh)
{
	...
	rh->this_host_bind_ipaddr = radius_malloc(sizeof(*rh->this_host_bind_ipaddr));
	if (rh->this_host_bind_ipaddr == NULL)
