...
#define ERR_PORTAL_STOPPED	0x3002
...
{
	if (user) {
		ems_l_trace("[portal]destroy user(%s, %s, %s, %s), exit st: 0x%x", 
				str_text(&user->name),
				str_text(&user->pass),
				str_text(&user->mac),
				str_text(&user->ip),
...
static ems_void user_timeout_cb(ems_timeout *timeout)
{
	...
	ems_l_trace("[portal]timer user(st: 0x%x, %s, %s), retry times: %d", user->st,
	...
	case portal_user_st_ack_auth:
	{
		if (ems_flag_like(user->flg, EMS_FLG_ONLINE))
		{
			...
			nic_sendmsg(ptl->ssid, ty_portal, ty_radius,   EMS_APP_CMD_RADIUS_LOGOUT, jobj);
		...
		ems_queue         *attr)
{
	...
	if (!sess) {
		if (user) {
			ems_l_warn("[portal session down]ptl (%s: %s), ack 0x%x, err: %d",
	...
	if (user) {
		ems_l_info("[portal %s]ptl (%s: %s), ack 0x%x, err: %d",
			str_text(&ptl->ssid->_iface->bssid),
			str_text(&user->ip), str_text(&user->name), auth->ty, auth->err);
		...
		ems_l_info("[portal %s]ptl, ack 0x%x, err: %d", 
			str_text(&ptl->ssid->_iface->bssid), auth->ty, auth->err);
...
ptl_evt_req_auth(ems_portal *ptl, ems_session *sess, portal_auth_hdr *auth, ems_queue *attr)
{
	...
	if (!user) {
		...
		ems_l_trace("[portal](current(0x%04x, 0x%04x), user(0x%04x, 0x%04x)", 
	...
	str_set(&user->mac, mac);
	{
		...
		json_object_object_add(jobj, "userip",   json_object_new_string(str_text(&user->ip)));
		json_object_object_add(jobj, "usermac",  json_object_new_string(ptl_mac_to_radius(&user->mac)));
		nic_sendmsg(ptl->ssid, ty_portal, ty_radius,   EMS_APP_CMD_RADIUS_AUTH, jobj);
...
ptl_evt_aff_ack_auth(ems_portal *ptl, ems_session *sess, portal_auth_hdr *auth, ems_queue *attr)
{
	...
	if (user && user->st == portal_user_st_ack_auth) {
...
ptl_process_rsp(ems_portal *ptl, ems_session *sess, ptl_recv_cb h)
{
	...
	if ((*buf & 0x00ff) == 0x01) {
...

		{
			...
			ems_l_info("[portal %s]evt ty: %d, serial: 0x%x, reqid:0x%x, ip: %s err: %d, nattr: %d",
				str_text(&ptl->ssid->_iface->bssid),
...
static ems_int ptl_reg_rsp(ems_portal *ptl, ems_session *sess, ems_queue *list)
{
	...
	if (vp && vp->val && !strcmp(vp->val, "register")) {
		if (!nic_app_run(ptl->ssid, ty_fw)) {
			nic_sendmsg(ptl->ssid, ty_portal, ty_fw, EMS_APP_START, NULL);
			{
				json_object *ary;
...
static ems_int portal_stopped(ems_portal *ptl, ems_session *sess, ems_uint flg)
{
	...
	ptl->lasterr = ERR_PORTAL_STOPPED;
	if (nic_app_run(ptl->ssid, ty_fw))
...
static ems_int portal_err(ems_portal *ptl, ems_session *sess, ems_uint flg)
{
	...
	if (sess) {
		...
		ems_l_info("[portal %s] session(%d) with [%s:%d] down, try again later", 
				str_text(&ptl->ssid->_iface->bssid),
				ems_sock_fd(&sess->sock),
				ems_sock_addr(&sess->sock),
...
ems_int  ptl_user_ntf_logout(ems_portal *ptl, portal_user *user)
{
	...
	if (user) {
		ems_l_info("[portal %s]ntf logout(%s:%s): current st: 0x%x", 
			str_text(&ptl->ssid->_iface->bssid),
			str_text(&user->name), str_text(&user->ip), user->st);
...
ems_int  ptl_user_auth_rsp(ems_portal *ptl, portal_user *user, ems_int err)
{
	...
	if (user) {
		...
		ems_l_info("[portal %s]auth rsp(%s:%s) current st: 0x%x", 
			str_text(&ptl->ssid->_iface->bssid),
			str_text(&user->name), str_text(&user->ip), user->st);

		switch(user->st) {
		case portal_user_st_aff_ack_auth:
			ems_l_error("[portal] portal server error, current status should be portal_user_st_req_auth");
		case portal_user_st_req_auth:
			break;
