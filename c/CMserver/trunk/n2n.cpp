...
pthread_mutex_t g_MutexForwardInfo;
map<SOCKET, ST_FORWARD_SERVER_INFO_INMAP> g_MapForwardInfo;

pthread_mutex_t g_MutexTicket;
map<SOCKET, list<string> > g_MapTicketInfo;

map<string, ST_PEER_INFO> g_MapPeerInfo;
...
const char* msg_type2str(uint16_t msg_type) {
  switch(msg_type) {
...
struct peer_info * find_peer_by_uuid( struct peer_info * list, const n2n_uuid uuid )
{
  while(list != NULL)
    {
      if( 0 == strcmp(uuid, list->uuid) )
        {
	  return list;
        }
      list = list->next;
...
void show_uuid( struct peer_info * list)
{
  while(list != NULL)
    {
      traceEvent(TRACE_INFO, "uuid = %s", list->uuid);
      list = list->next;
    }
