...
def get_fileinfo(fl):
	...
	ary = filename.split('-comb-')

	if len(ary) != 2 or ary[0] != "Yingke-WiFi-G1" or ary[1][-4:] != ".bin":
	...
	return (ary[0].replace("-"," "), ary[1][:-4], chk)
...
def update_file_config(dev, ver, chk, fl):
	...
	if not fp:
		...
		return None
	ctx = "rom.path=%s\nrom.checksum=%s\nrom.version=%s\nrom.dev=%s\n"%(
			fl.split(os.getenv("DOCUMENT_ROOT"))[-1],chk, ver, dev)
