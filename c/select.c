...
#define sock_puts(fd, buf) send(fd, buf, strlen(buf), 0)
#define sock_gets(fd, buf, len) recv(fd, buf, len, 0)

void sig_handler(int sig)
{
	printf("signal here: %d", sig);
}
...
void handle_new_connection() {
	...
	if (connection < 0) {
		...
		switch(lasterr) {
			...
			printf("child process: %ld , accept failed: errno: %d\n", 
					(long)getpid(), lasterr);
				return;
		}
		printf("accept: child process: %ld , accept failed: %s\n", 
					(long)getpid(), strerror(lasterr));
		exit(EXIT_FAILURE);
	}
...
void read_socks() {
	...
	printf("child process id: %ld, read sock\n", (long) getpid());
