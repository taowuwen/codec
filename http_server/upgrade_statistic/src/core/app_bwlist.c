...
static ems_int bwlist_run(app_module *mod, ems_int run)
{
	...
	if (run) {
		...
		ems_l_trace("bwlist running here");
...
static ems_int bwlist_process_rule(app_module *mod, json_object *root)
{
	ems_l_trace("do not use this any more, obsoluted");
	return EMS_OK;
}
...
bwlist_server_rules_update(app_module *mod, json_object *root)
{
	...
	return ems_app_process(ty_bwlist, ty_fw, EMS_APP_SERVER_RULES_UPDATE, NULL);
}
...
{
	ems_l_trace("bwlist evt: 0x%x, from: 0x%x, args: %s", 
...
app_module app_bwlist = 
{
	...
	.process_rule  = bwlist_process_rule,
	.version_match = NULL,
	.install       = NULL
};
